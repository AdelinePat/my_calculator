...
cursor = {
    ...
    "error_red_start" : "\033[0;1;31m",
    "calc_box_style_start" : "\033[0;1;38;5;129m",
    "results_style_start" : "\033[0;1;38;5;82m",
    "historic_style_start" : "\033[0;1;2;38;5;82m",
...
def setup_printable_fields():
    print(
        ...
        f"{cursor_line(1)}{" ":^{calc_padding}}{calc_box['angle_tl']}",
        ...
        f"{cursor_line(2)}{" ":^{calc_padding}}{calc_box['vertical']}",
        f"{" ":^{calc_width}}{calc_box['vertical_bis']}",
        f"{" ":^{historic_width}}{calc_box['vertical']}",
        f"{cursor['light_line_clear']}",

        f"{cursor_line(3)}{" ":^{calc_padding}}{calc_box['vertical']}",
        f"{cursor['instructions_style_start']}{" Résultat :":<{calc_width}}",
        f"{cursor['calc_box_style_start']}{calc_box['vertical_bis']}",
        f"{" ":^{historic_width}}{calc_box['vertical']}",
        f"{cursor['light_line_clear']}",
        
        f"{cursor_line(4)}{" ":^{calc_padding}}{calc_box['vertical']}",
        f"{" ":^{calc_width}}{calc_box['vertical_bis']}",
        f"{cursor['dim_start']}{"":{calc_box['horizontal']}^{historic_width}}",
        f"{cursor['bold_dim_end']}{cursor['bold_start']}{calc_box['vertical']}",
        f"{cursor['light_line_clear']}",
        
        f"{cursor_line(5)}{" ":^{calc_padding}}{calc_box['vertical']}",
        f"{" ":^{calc_width}}{calc_box['vertical_bis']}",
        f"{" ":^{historic_width}}{calc_box['vertical']}",
        f"{cursor['light_line_clear']}",
        
        f"{cursor_line(6)}{" ":^{calc_padding}}{calc_box['vertical']}",
        f"{" ":^{calc_width}}{calc_box['vertical_bis']}",
        f"{" ":^{historic_width}}{calc_box['vertical']}",
        f"{cursor['light_line_clear']}",
        
        f"{cursor_line(7)}{" ":^{calc_padding}}{calc_box['horizontal_bis_>']}",
        f"{"":{calc_box['horizontal_bis']}^{calc_width}}{calc_box['vertical_bis_<']}",
        f"{cursor['dim_start']}{"":{calc_box['horizontal']}^{historic_width}}",
        f"{cursor['bold_dim_end']}{cursor['bold_start']}{calc_box['vertical']}",
        f"{cursor['light_line_clear']}",
        
        f"{cursor_line(8)}{" ":^{calc_padding}}{calc_box['vertical']}",
        f"{" ":^{calc_width}}{calc_box['vertical_bis']}",
        f"{" ":^{historic_width}}{calc_box['vertical']}",
        f"{cursor['light_line_clear']}",
        
        f"{cursor_line(9)}{" ":^{calc_padding}}{calc_box['vertical']}",
        f"{" ":^{calc_width}}{calc_box['vertical_bis']}",
        f"{" ":^{historic_width}}{calc_box['vertical']}",
        f"{cursor['light_line_clear']}",
        
        f"{cursor_line(10)}{" ":^{calc_padding}}{calc_box['vertical']}",
        f"{" ":^{calc_width}}{calc_box['vertical_bis']}",
        f"{cursor['dim_start']}{"":{calc_box['horizontal']}^{historic_width}}",
        f"{cursor['bold_dim_end']}{cursor['bold_start']}{calc_box['vertical']}",
        f"{cursor['light_line_clear']}",
        
        f"{cursor_line(11)}{" ":^{calc_padding}}{calc_box['vertical']}",
        f"{" ":^{calc_width}}{calc_box['vertical_bis']}",
        f"{" ":^{historic_width}}{calc_box['vertical']}",
        f"{cursor['light_line_clear']}",
        
        f"{cursor_line(12)}{" ":^{calc_padding}}{calc_box['vertical']}",
        f"{cursor['instructions_style_start']}{"Entrez votre calcul":^{calc_width}}",
        f"{cursor['calc_box_style_start']}{calc_box['vertical_bis']}",
        f"{" ":^{historic_width}}{calc_box['vertical']}",
        f"{cursor['light_line_clear']}",
        
        f"{cursor_line(13)}{" ":^{calc_padding}}{calc_box['vertical_>']}",
        f"{"":{calc_box['horizontal']}^{calc_width}}{calc_box['vertical_bis_^']}",
        f"{"":{calc_box['horizontal']}^{historic_width}}{calc_box['vertical']}",
        f"{cursor['light_line_clear']}",
        
        f"{cursor_line(14)}{" ":^{calc_padding}}{calc_box['vertical']}",
        f"{" ":^{calc_width}} ",
        f"{" ":^{historic_width}}{calc_box['vertical']}",
        f"{cursor['light_line_clear']}",
        
        f"{cursor_line(15)}{" ":^{calc_padding}}{calc_box['vertical']}",
        f"{" ":^{calc_width}} ",
        f"{" ":^{historic_width}}{calc_box['vertical']}",
        f"{cursor['light_line_clear']}",
        
        f"{cursor_line(16)}{" ":^{calc_padding}}{calc_box['vertical']}",
        f"{" ":^{calc_width}} ",
        f"{" ":^{historic_width}}{calc_box['vertical']}",
        f"{cursor['light_line_clear']}",
        
        f"{cursor_line(17)}{" ":^{calc_padding}}{calc_box['angle_bl']}",
...
def historic_data_print(historic):
    for row in range(0,4):
        x_coord = row*3+3
        y_coord = calc_padding+calc_width+4
        if type(historic[row][1]) == float and historic[row][1] != "":
            print(
                f"{cursor_line(x_coord,y_coord)}{cursor['historic_style_start']}",
...
def result_data_print(calc_result):
    if type(calc_result) == float:
        print(
            f"{cursor_line(5, calc_padding+2)}{cursor['results_style_start']}",
...
def calc_input_data_print(last_input):
    print(
        f"{cursor_line(9, calc_padding+3)}{cursor['dim_start']}",
        f"{last_input[:calc_width-6]:>{calc_width-4}}",
        f" {"=" if last_input != "" else " "}{cursor['style_finish']}",

        sep="", end="", flush=True)
    return input(cursor_line(10,calc_padding+4))
...
def error_1_first_operator_print():
    print(
        f"{cursor_line(15,calc_padding+2)}{cursor['error_red_start']}",
...
def error_clear_print():
    print(
        f"{cursor_line(15,calc_padding+1)}{" ":^{calc_width+1+historic_width}}",
        sep="", end="", flush=True)